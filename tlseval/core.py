"""Core scoring: voxelise, match, score, classify failures.

Input is a single LAZ/LAS file containing at minimum:
  - x, y, z coordinates
  - treeID:  ground truth instance labels (0 = unlabeled)
  - predID:  predicted instance labels (0 = unlabeled)

Optional:
  - completely_inside / completelyInside: 1 = tree fully within the plot
    boundary. When present, only those trees are evaluated unless all_trees
    is set. Both spellings are accepted; see INSIDE_FIELDS.

Every run writes an `evaluation_config` header into the output CSV. Results
produced under different settings are not comparable, so the settings travel
with the numbers.
"""
...
class TlsEvalError(Exception):
    """Anything the caller can fix: a missing field, an unreadable file, a
    mismatched config. Library code raises this rather than calling SystemExit,
    so a batch run can record one bad plot and carry on instead of dying."""
...
# Accepted spellings of the boundary flag, in priority order. TreeScanPL ships
# `completelyInside`; earlier versions of this script looked only for
# `completely_inside`, so on the real data the filter silently did nothing and
# boundary-clipped trees were scored as if whole.
INSIDE_FIELDS = ("completely_inside", "completelyInside")
...
def classify_failures(
    ...
    for tid in tree_ids:
        ...
        # Coerce to Python bool: numpy scalars leak into the CSV writer and
        # compare unequal to True under `is`, which surprises callers.
        out[tid] = {"missed": bool(missed), "split": bool(split),
                    "merged": bool(merged)}
    return out
...
def evaluate(
    ...
    for field in (gt_field, pred_field):
        try:
            ...
            raise TlsEvalError(
                f"field '{field}' not found in {path}; available: "
                + ", ".join(d.name for d in las.point_format.dimensions)
            )

    gt_ids = np.asarray(las[gt_field]).astype(np.int64)
    pred_ids = np.asarray(las[pred_field]).astype(np.int64)

    completely_inside, inside_field = None, None
    for name in INSIDE_FIELDS:
        try:
            completely_inside = np.asarray(las[name])
            inside_field = name
            break
        except Exception:
            continue
    ...
    df.attrs["n_gt_instances"] = n_all
    df.attrs["inside_field"] = inside_field
    df.attrs["n_excluded_boundary"] = n_all - len(tree_ids)
    return df
...
def config_from(voxel_size=DEFAULTS["voxel_size"], gt_field=DEFAULTS["gt_field"],
                pred_field=DEFAULTS["pred_field"], all_trees=DEFAULTS["all_trees"],
                dominance_threshold=DEFAULTS["dominance_threshold"],
                fragment_threshold=DEFAULTS["fragment_threshold"]) -> dict:
    """The dict stamped into every output file."""
    return {
